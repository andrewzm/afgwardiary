...
%Basis decomposition
load('../TextS6_BasisSelection/AFGBasis')
...
Thetainfo(1).Meanprecmat = 1/(sigmaW^2)*eye(Basis.nx);
Thetainfo(1).best = [-0.01,-0.01];
...
EstIntensity = DiggleContSpace(spikeAllWeekData(1).Coords,Constants);
...
b = Thetainfo.best;
bvar = Thetainfo.varb;
pop_den = Constants.Pop_density;
dist_city = Constants.Dist_to_city;
